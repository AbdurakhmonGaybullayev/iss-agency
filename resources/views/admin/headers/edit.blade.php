...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.headers.update", [$header->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="videos">{{ trans('cruds.header.fields.videos') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.header.fields.videos_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    Dropzone.options.videosDropzone = {
        ...
        success: function (file, response) {
            $('form').find('input[name="videos"]').remove()
        ...
        init: function () {
            @if(isset($header) && $header->videos)
            var file = {!! json_encode($header->videos) !!}
            this.options.addedfile.call(this, file)
            this.options.thumbnail.call(this, file, file.preview)
            file.previewElement.classList.add('dz-complete')
