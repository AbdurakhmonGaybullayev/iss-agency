...
@php $seo = \App\Models\SeoTable::where('name','videos')->first(); @endphp
@section('title')
    {{$seo['title_'.$locale]}}
@endsection
@section('meta')
...
@section('meta')
    @if(isset($video))
    <meta property="og:url" content="{{URL::full()}}">
    <meta property="og:title" content="{{$video['title_'.$locale]}} - issagency.uz">
    <meta property="og:description" content="{{$video['short_description_'.$locale]}}">
    <meta property="og:image" content="{{$video->cover->getUrl()}}">
    <meta property="og:type" content="article">
    <meta name="description" content="{{$video->cover->getUrl()}}">
    <meta name="keywords" content="{{$video['seo_keywords_'.$locale]}}">
@endif
@endsection
        ...
        <div class="modal-dialog modal-dialog-centered modal-width" role="document">
            <div style="padding: 0 10px 10px 10px" class="modal-content">
                <div id="modal-header" class="modal-header">
                    ...
                    <button type="button" class="close" onclick="document.getElementById('video').pause()" data-dismiss="modal" aria-label="Close">
                            ...
                            <div class="single-gallery-inner">
                                <div
                                    ...
                                    <a class="play-icon" href="javascript:void(0)"
                                       onclick="playVideo('{{$video->file->getUrl()}}','{{ str_replace("'","\'",$video['title_'.$locale]) }}')"><i
                                ...
                                <div style="padding-bottom: 10px" class="details">
                                    <h6 style="color: white">
                                        <a href="javascript:void(0)"
                                           onclick="playVideo('{{$video->file->getUrl()}}','{{ str_replace("'","\'",$video['title_'.$locale]) }}')">{{$video['title_'.$locale]}}</a>
                                    </h6>
                                </div>
                            </div>
                        </div>
