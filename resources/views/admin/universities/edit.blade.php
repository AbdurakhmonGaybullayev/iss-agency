        ...
        <form method="POST" action="{{ route("admin.universities.update", [$university->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('top') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="top" id="top" value="1" {{ $university->top || old('top', 0) === 1 ? 'checked' : '' }} required>
                    <label class="required form-check-label" for="top">{{ trans('cruds.university.fields.top') }}</label>
                </div>
                @if($errors->has('top'))
                    <div class="invalid-feedback">
                        {{ $errors->first('top') }}
