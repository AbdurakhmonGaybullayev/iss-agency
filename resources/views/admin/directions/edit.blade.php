    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.directions.update", [$direction->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_uz') ? 'is-invalid' : '' }}" type="text" name="name_uz" id="name_uz" value="{{ old('name_uz', $direction->name_uz) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_ru') ? 'is-invalid' : '' }}" type="text" name="name_ru" id="name_ru" value="{{ old('name_ru', $direction->name_ru) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text" name="name_en" id="name_en" value="{{ old('name_en', $direction->name_en) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('programms') ? 'is-invalid' : '' }}" name="programms[]" id="programms" multiple required>
                    @foreach($programms as $id => $programm)
                        <option value="{{ $id }}" {{ (in_array($id, old('programms', [])) || $direction->programms->contains($id)) ? 'selected' : '' }}>{{ $programm }}</option>
                    @endforeach
                </select>
