...
@section('content')
            ...
            <form method="POST" action="{{ route("admin.branches.update", [$branch->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('working_days_from') ? 'is-invalid' : '' }}" name="working_days_from" id="working_days_from" required>
                        ...
                        @foreach(App\Models\Branch::WEEKDAYS as $key => $label)
                            <option value="{{ $key }}" {{ old('working_days_from', $branch->working_days_from) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('working_days_to') ? 'is-invalid' : '' }}" name="working_days_to" id="working_days_to" required>
                        ...
                        @foreach(App\Models\Branch::WEEKDAYS as $key => $label)
                            <option value="{{ $key }}" {{ old('working_days_to', $branch->working_days_to) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('working_hours_from') ? 'is-invalid' : '' }}" type="time" name="working_hours_from" id="working_hours_from" value="{{ old('working_hours_from', $branch->working_hours_from) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('working_hours_to') ? 'is-invalid' : '' }}" type="time" name="working_hours_to" id="working_hours_to" value="{{ old('working_hours_to', $branch->working_hours_to) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select onchange="region_abduvohid()" class="form-control {{ $errors->has('region') ? 'is-invalid' : '' }}" name="region" id="region" required>
                        ...
                        @foreach(App\Models\Branch::REGION_SELECT as $key => $label)
                            <option  data-value="{{$label['city']}}" value="{{ $label['city'] }}" {{ old('region', $branch->region)  === $label['city'] ? 'selected' : '' }}>{{ $label['city'] }}</option>
                        @endforeach
    ...
    <script>
        ...
        let active_option=$select.options[$select.selectedIndex].dataset.value;
