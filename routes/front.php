...
Route::prefix('/{lang}')->group(function () {
    ...
    Route::get('/videos',function (){
        ...
        return view('front.videos.index',[
            'background'=>\App\Models\Header::first()->videos->getUrl(),
            'videos'=>$videos,
            'contact'=>$contact,
            'testimonials'=>\App\Models\Testimonial::take(9)->orderBy('created_at', 'desc')->get(),
            'news'=>\App\Models\News::orderBy('created_at', 'desc')->get(),
    ...
    Route::get('/branch',function (){
        ...
        if (\Illuminate\Support\Facades\Auth::user()){
            $user = \Illuminate\Support\Facades\Auth::user();
            $contact = \App\Models\Contact::where('branch_id',$user->branch_id)->first();
            $branches = \App\Models\Branch::join('contacts','contacts.branch_id','branches.id')->where('branches.id','!=',$user->branch_id)->select('branches.*','contacts.phone_number')->orderBy('branches.number', 'desc')->get()->unique();
    ...
    Route::get('/dashboard/profile-edit',function (){
        ...
        return view('front.dashboard.profile-edit.index',[
            ...
            'contact'=>$contact,
            'news'=>\App\Models\News::orderBy('created_at', 'desc')->get(),
            'branches'=>\App\Models\Branch::join('contacts','contacts.id','branches.id')->select('branches.*')->orderBy('branches.name_uz', 'desc')->get(),
        ]);
    })->name('profile-edit');
